# -*- coding: utf-8 -*-
"""
ANÁLISIS DEL DATO — RANDOM FOREST 
...
RUTA_GRAFICOS = r'C:\Users\isabe\Desktop\TFG\ENTREGA 2\GRAFICOS\RF'
...
print("\n" + "="*80)
print("PASO 1: RANDOM FOREST (MODELO PRINCIPAL)")
print("="*80)
...
# Hiperparámetros conservadores para evitar sobreajuste con n=81:
#   - max_depth=4: limita la profundidad de cada árbol, fuerza generalización
#   - min_samples_leaf=5: cada hoja necesita al menos 5 obs (6% del total)
#   - min_samples_split=10: se necesitan 10 obs mínimo para hacer una división
# Estos valores priorizan la estabilidad sobre el ajuste en muestra
...
print(f"    min_samples_leaf: {HIPERPARAMETROS['min_samples_leaf']} observaciones")
print(f"    min_samples_split:{HIPERPARAMETROS['min_samples_split']} observaciones")
...
for fold, (train_idx, test_idx) in enumerate(tscv.split(X), 1):
    ...
    resultados_folds.append({
            ...
            'R² Train': r2_train,
            'R² Test': r2_test,
            # guardamos también los arrays para los gráficos
...
# Se calcula sobre el modelo entrenado en muestra completa

rf_completo = RandomForestRegressor(**HIPERPARAMETROS)
rf_completo.fit(X, y)

importancias = rf_completo.feature_importances_
...
tabla_png(
    df_cv[cols_tabla_cv],
    'Random Forest — Métricas de validación temporal por fold',
    'TimeSeriesSplit 4 folds — mora_hogares en M€',
    os.path.join(RUTA_GRAFICOS, 'tabla_cv_rf.png')
)
...
tabla_png(
    ...
    os.path.join(RUTA_GRAFICOS, 'tabla_feature_importance_rf.png'),
...
colores = [COLOR_PRINCIPAL] * len(df_importance)
...
# Poner porcentaje en cada barra
...
ax.set_title(
    'Random Forest — Feature Importance (muestra completa)\n'
...
plt.tight_layout()
ruta_fi = os.path.join(RUTA_GRAFICOS, 'feature_importance_rf.png')
plt.savefig(ruta_fi, dpi=150, bbox_inches='tight', facecolor='white')
plt.close()
print(f"  Grafico guardado: feature_importance_rf.png")
...
fig.suptitle(
    'Random Forest — Real vs Predicho por fold\n'
    '(validación temporal, TimeSeriesSplit 4 folds)',
    fontsize=12, fontweight='bold'
)
...
plt.tight_layout()
ruta_pred = os.path.join(RUTA_GRAFICOS, 'predicciones_rf_folds.png')
plt.savefig(ruta_pred, dpi=150, bbox_inches='tight', facecolor='white')
plt.close()
print(f"  Grafico guardado: predicciones_rf_folds.png")
...
fig.suptitle(
    'Random Forest — Evolución de métricas por fold (validación temporal)\n'
...
plt.tight_layout()
ruta_metricas = os.path.join(RUTA_GRAFICOS, 'cv_metricas_rf.png')
plt.savefig(ruta_metricas, dpi=150, bbox_inches='tight', facecolor='white')
plt.close()
print(f"  Grafico guardado: cv_metricas_rf.png")
...
print(f"    tabla_cv_rf.png")
print(f"    tabla_feature_importance_rf.png")
print(f"    feature_importance_rf.png")
print(f"    predicciones_rf_folds.png")
print(f"    cv_metricas_rf.png")
print("="*80 + "\n")